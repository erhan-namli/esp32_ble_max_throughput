...
uint8_t data_buffer[41];

static bool is_connect = false;
///Declare the static function
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param);

static void fill_test_data(data_t *data, uint32_t id, uint32_t ads[8], uint32_t timestamp, uint32_t rskin, uint32_t heartrate, uint32_t battery, uint32_t temperature);
...
uint16_t calculateCRC(data_t *data) {
  // CRC parameters
...
static void fill_test_data(data_t *data, uint32_t id, uint32_t ads[8], uint32_t timestamp, uint32_t rskin, uint32_t heartrate, uint32_t battery, uint32_t temperature)
{
	...
	data->type.ads[0] = ads[0];
	data->type.ads[1] = ads[0] >> 8;
	data->type.ads[2] = ads[0] >> 16;

	data->type.ads[3] = ads[1];
	data->type.ads[4] = ads[1] >> 8;
	data->type.ads[5] = ads[1] >> 16;

	data->type.ads[6] = ads[2];
	data->type.ads[7] = ads[2] >> 8;
	data->type.ads[8] = ads[2] >> 16;

	data->type.ads[9] = ads[3];
	data->type.ads[10] = ads[3] >> 8;
	data->type.ads[11] = ads[3] >> 16;

	data->type.ads[12] = ads[4];
	data->type.ads[13] = ads[4] >> 8;
	data->type.ads[14] = ads[4] >> 16;

	data->type.ads[15] = ads[5];
	data->type.ads[16] = ads[5] >> 8;
	data->type.ads[17] = ads[5] >> 16;

	data->type.ads[18] = ads[6];
	data->type.ads[19] = ads[6] >> 8;
	data->type.ads[20] = ads[6] >> 16;

	data->type.ads[21] = ads[7];
	data->type.ads[22] = ads[7] >> 8;
	data->type.ads[23] = ads[7] >> 16;

	data->type.timestamp[0] = timestamp;
	data->type.timestamp[1] = timestamp >> 8;
	data->type.timestamp[2] = timestamp >> 16;

	data->type.rskin[0] = rskin;
	data->type.rskin[1] = rskin >> 8;
	data->type.rskin[2] = rskin >> 16;

	data->type.heartrate[0] = heartrate;
	data->type.heartrate[1] = heartrate >> 8;
	data->type.heartrate[2] = heartrate >> 16;

	data->type.battery[0] = battery;
	data->type.battery[1] = battery >> 8;

	data->type.temperature[0] = temperature;
	data->type.temperature[1] = temperature >> 8;

    uint16_t crc = calculateCRC(data);

    ESP_LOGE(GATTS_TAG, "%d crc", crc);

    data->type.crc[0] = crc; // High byte
    data->type.crc[1] = crc>>8; // Low byte
...
void throughput_server_task(void *param)
{
	...
	static uint32_t ads[8] = {10, 10, 10, 10, 10, 10, 10, 10};
	static uint32_t timestamp = 0;
	static uint32_t rskin = 0;
	static uint32_t heartrate = 0;
	static uint32_t battery = 0;
	static uint32_t temperature = 55;

    fill_test_data(&data_storage, packet_id, ads, timestamp, rskin, heartrate, battery, temperature);
    ...
    while(1) {
        ...
        if (!can_send_notify) {
            ...
            if (is_connect) {
            	...
            	memcpy(&data_buffer, data_storage.bytes, 41);
                ...
                if(free_buff_num > 0) {
                    for( ; free_buff_num > 0; free_buff_num--) {
                        esp_ble_gatts_send_indicate(gl_profile_tab[PROFILE_A_APP_ID].gatts_if,
                                                    ...
                                                    sizeof(data_buffer), data_buffer, false);
                    }
